...
function templateContent(type, slug, suffix) {
  let tpl = fs.readFileSync(path.join(TEMPLATE_DIR, `${type}.md`), 'utf-8');
...
function normalizeCommitUrl(commitUrl) {
  ...
  if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error('commit URL must be a valid http(s) URL');
  return value;
}
...
async function createChecklistItem(db, projectId, documentId, { text, status, commitUrl } = {}) {
  ...
  const result = await db.run('INSERT INTO checklist_items (document_id, position, text, status, commit_url) VALUES (?, ?, ?, ?, ?) RETURNING id', [documentId, nextPosition, normalizeChecklistText(text), status === undefined ? 'DRAFT' : status, normalizeCommitUrl(commitUrl)]);
...
async function updateChecklistItem(db, projectId, itemId, changes = {}) {
  ...
  if (changes.commitUrl !== undefined) { updates.push('commit_url = ?'); values.push(normalizeCommitUrl(changes.commitUrl)); }
...
module.exports = {
  ...
  listChecklistItems, createChecklistItem, updateChecklistItem, deleteChecklistItem, listRelations, relate,
};
