...
function normalizeSearchStatuses(status) {
...
async function validateSearchFilters(db, { type, status } = {}) {
  if (type && !DOC_TYPES.includes(type) && type !== 'constitution') {
    throw new Error(`Invalid type '${type}'. Valid: ${[...DOC_TYPES, 'constitution'].join(', ')}`);
  }
  for (const value of normalizeSearchStatuses(status)) {
    await statuses.assertStatus(db, value);
  }
}
...
async function resolveStatus(db, status) {
  if (status != null && String(status).trim() !== '') {
    await statuses.assertStatus(db, status);
    return String(status).trim();
  }
  const all = await statuses.list(db);
  return all.length ? all[0].code : 'DRAFT';
}
...
async function search(db, projectId, { query = '', type, status } = {}) {
  await validateSearchFilters(db, { type, status });
  const statuses = normalizeSearchStatuses(status);
  const where = ['project_id = ?'];
  const params = [projectId];
...
async function create(db, projectId, { type, slug, suffix, status, content }) {
  ...
  const finalStatus = await resolveStatus(db, status);
...
async function setStatus(db, projectId, id, status) {
  const finalStatus = await resolveStatus(db, status);
  const result = await db.run('UPDATE documents SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND project_id = ?', [finalStatus, id, projectId]);
  return result.changes > 0;
}

async function rename(db, projectId, id, slug) {
  const value = String(slug == null ? '' : slug).trim();
  if (!value) throw new Error('slug is required');
  const document = await db.get('SELECT id, slug, type, suffix FROM documents WHERE id = ? AND project_id = ?', [id, projectId]);
  if (!document) return undefined;
  const conflict = await db.get(
    "SELECT id FROM documents WHERE project_id = ? AND slug = ? AND type = ? AND COALESCE(suffix, '') = COALESCE(?, '') AND id != ?",
    [projectId, value, document.type, document.suffix, id],
  );
  if (conflict) {
  ...
  await db.run('UPDATE documents SET slug = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND project_id = ?', [value, id, projectId]);
  return get(db, projectId, id);
}

async function remove(db, projectId, id) {
  const result = await db.run('DELETE FROM documents WHERE id = ? AND project_id = ?', [id, projectId]);
  return result.changes > 0;
}
...
async function createChecklistItem(db, projectId, documentId, { text, status, commitUrl } = {}) {
  ...
  if (!document) return undefined;
  const itemStatus = await resolveStatus(db, status);
  ...
  const result = await db.run('INSERT INTO checklist_items (document_id, position, text, status, commit_url) VALUES (?, ?, ?, ?, ?) RETURNING id', [documentId, nextPosition, normalizeChecklistText(text), itemStatus, normalizeCommitNote(commitUrl)]);
...
async function updateChecklistItem(db, projectId, itemId, changes = {}) {
  ...
  if (changes.status !== undefined) { const finalStatus = await resolveStatus(db, changes.status); updates.push('status = ?'); values.push(finalStatus); }
...
module.exports = {
  DOC_TYPES, RELATION_TYPES, CHECKLIST_LABELS, checklistLabel, templateContent, resolveStatus,
  list, search, resolve, get, create, setStatus, rename, remove, writeContent,
