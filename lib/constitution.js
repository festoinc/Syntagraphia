...
/** Read the project's singleton constitution row (without relations). */
function get(db, projectId) {
  return db.prepare('SELECT * FROM documents WHERE project_id = ? AND type = ? ORDER BY id LIMIT 1').get(projectId, TYPE);
}

/** Upsert the constitution content (singleton, scoped by project). Returns the row. */
function set(db, projectId, content) {
  const existing = get(db, projectId);
  if (existing) {
  ...
  const result = db.prepare(
    'INSERT INTO documents (project_id, slug, type, suffix, status, content) VALUES (?, ?, ?, NULL, ?, ?)'
  ).run(projectId, SLUG, TYPE, 'DONE', content == null ? '' : content);
