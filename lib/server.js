...
async function createApp() {
  ...
  app.get('/api/statuses', ah(async (_req, res) => {
    ...
    res.json(rows.map((r) => ({ ...r, usage: usage[r.code] || 0 })));
  }));

  app.post('/api/statuses', ah(async (req, res) => {
    const { code, label } = req.body || {};
    try {
      const created = await statuses.add(db, { code, label });
      res.status(201).json(created);
    } catch (error) {
      if (error.code === 'EXISTS') return res.status(409).json({ error: error.message, id: error.id });
      res.status(400).json({ error: error.message });
    }
  }));

  app.put('/api/statuses/:code', ah(async (req, res) => {
    const { code, label } = req.body || {};
    try {
      const renamed = await statuses.rename(db, req.params.code, { code, label });
      res.json(renamed);
    } catch (error) {
      if (error.code === 'EXISTS') return res.status(409).json({ error: error.message, id: error.id });
      res.status(400).json({ error: error.message });
  ...
  app.delete('/api/statuses/:code', ah(async (req, res) => {
    try {
      const removed = await statuses.remove(db, req.params.code);
      res.json(removed);
    } catch (error) {
      if (/still use it/.test(error.message)) return res.status(409).json({ error: error.message });
      res.status(400).json({ error: error.message });
    }
  }));
