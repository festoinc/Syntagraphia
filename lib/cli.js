...
// ── arg parsing ───────────────────────────────────────────────
const VALUE_FLAGS = new Set(['--project', '--type', '--status', '--suffix', '--file', '--port', '--constitution-file', '--name', '--text', '--commit', '--url', '--label']);
...
Commands:
  ...
  doc delete <id|slug> --project <p>           Delete a document and its attached data
  ...
  status list                                 List the global status vocabulary
  status add <CODE> [--label <Label>]         Add a custom status
  status rename <OLD> <NEW> [--label <Label>] Rename a status, updating docs that use it
  status remove <CODE>                        Remove a status (fails while still in use)
...
async function cmdDocDelete(db, projectId, flags, positionals) {
  const documentRef = positionals[0];
  if (!documentRef) { err('doc delete requires <id|slug>'); return 1; }
  const row = await resolveDocument(db, projectId, documentRef);
  if (!row) return 1;
  const deleted = await docs.remove(db, projectId, row.id);
  if (!deleted) { err(`document '${documentRef}' not found in this project`); return 1; }
  const result = { id: row.id, slug: row.slug, type: row.type, suffix: row.suffix, deleted: true };
  if (flags['--json']) { printJson(result); return 0; }
  out(`✓ Deleted [${row.id}] ${row.type} — ${docLabel(row)}`);
  return 0;
}
...
async function cmdStatusList(db, flags) {
  const rows = await statuses.list(db);
  const usage = {};
  for (const r of await db.all('SELECT status, COUNT(*) AS count FROM documents GROUP BY status')) {
    usage[r.status] = (usage[r.status] || 0) + Number(r.count);
  }
  for (const r of await db.all('SELECT status, COUNT(*) AS count FROM checklist_items GROUP BY status')) {
    usage[r.status] = (usage[r.status] || 0) + Number(r.count);
  }
  if (flags['--json']) {
    printJson(rows.map((r) => ({ ...r, usage: usage[r.code] || 0 })));
    return 0;
  }
  out('CODE            LABEL           USAGE');
  out('--------------  --------------  -----');
  for (const r of rows) out(`${r.code.padEnd(14)}  ${r.label.padEnd(15)}  ${usage[r.code] || 0}`);
  return 0;
}

async function cmdStatusAdd(db, flags, positionals) {
  const code = positionals[0];
  if (!code) { err('status add requires <CODE>'); return 1; }
  try {
    const created = await statuses.add(db, { code, label: flags['--label'] });
    if (flags['--json']) { printJson(created); return 0; }
    out(`✓ Added status '${created.code}' (${created.label})`);
    return 0;
  } catch (e) {
    if (e.code === 'EXISTS') { err(`status '${code}' already exists`); return 1; }
    err(e.message); return 1;
  }
}

async function cmdStatusRename(db, flags, positionals) {
  const [oldCode, newCode] = positionals;
  if (!oldCode || !newCode) { err('status rename requires <OLD> <NEW>'); return 1; }
  try {
    const renamed = await statuses.rename(db, oldCode, { code: newCode, label: flags['--label'] });
    if (flags['--json']) { printJson({ id: renamed.id, code: renamed.code, label: renamed.label }); return 0; }
    out(`✓ Renamed status '${oldCode}' → '${renamed.code}' (${renamed.label})`);
    return 0;
  } catch (e) {
    if (e.code === 'EXISTS') { err(`status '${newCode}' already exists`); return 1; }
    err(e.message); return 1;
  }
}

async function cmdStatusRemove(db, flags, positionals) {
  const code = positionals[0];
  if (!code) { err('status remove requires <CODE>'); return 1; }
  try {
    const removed = await statuses.remove(db, code);
    if (flags['--json']) { printJson(removed); return 0; }
    out(`✓ Removed status '${removed.code}'`);
    return 0;
  } catch (e) { err(e.message); return 1; }
}
...
async function cmdStatus(db, projectId, flags) {
  ...
  out(`\nBy status:`);
  const statusRows = await statuses.list(db);
  for (const s of statusRows) {
    out(`  ${s.code.padEnd(14)} ${(byStatus[s.code] || 0)}`);
  }
    ...
    switch (cmd) {
      ...
      case 'doc': {
        ...
        switch (sub) {
          ...
          case 'delete':     rc = await cmdDocDelete(db, p.id, flags, rest); break;
          ...
          default:           err(`unknown doc subcommand '${sub}'. Try: list, show, create, set-status, rename, delete, checklist, update, write, edit`); rc = 1;
        }
        await db.close?.();
        return rc;
      }
      ...
      case 'status': {
        const db = await openDb();
        const sub = positionals[1];
        if (sub === 'list' || sub === 'add' || sub === 'rename' || sub === 'remove') {
          let rc;
          if (sub === 'list') rc = await cmdStatusList(db, flags);
          else if (sub === 'add') rc = await cmdStatusAdd(db, flags, positionals.slice(2));
          else if (sub === 'rename') rc = await cmdStatusRename(db, flags, positionals.slice(2));
          else rc = await cmdStatusRemove(db, flags, positionals.slice(2));
          await db.close?.();
          return rc;
        }
        const p = await requireProject(db, flags);
