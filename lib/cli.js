...
// ── arg parsing ───────────────────────────────────────────────
const VALUE_FLAGS = new Set(['--project', '--type', '--status', '--suffix', '--file', '--port', '--constitution-file', '--name']);
...
function resolveDocument(db, projectId, documentRef) {
...
function cmdDocUpdate(db, projectId, flags, positionals) {
  const [documentRef, filePath] = positionals;
  if (!documentRef || !filePath) {
    err('doc update requires <id|slug> <file.md>');
    return 1;
  }
  const row = resolveDocument(db, projectId, documentRef);
  if (!row) return 1;
  let content;
  try {
    ...
    switch (cmd) {
      ...
      case 'doc': {
        ...
        switch (sub) {
          ...
          case 'update':     rc = cmdDocUpdate(db, p.id, flags, rest); break;
          case 'write':      rc = cmdDocWrite(db, p.id, flags, rest); break;
          case 'edit':       rc = cmdDocEdit(db, p.id, flags, rest); break;
          default:           err(`unknown doc subcommand '${sub}'. Try: list, show, create, set-status, update, write, edit`); rc = 1;
        }
        db.close?.();
        return rc;
      }
